...
VS_OUT VS_Main(VS_IN input)
{
    ...
    output.viewPos = mul(float4(worldPos, 1.f), matView);
    output.uv = input.uv;
    output.id = input.id;
...
void GS_Main(point VS_OUT input[1], inout TriangleStream<GS_OUT> outputStream)
{
    ...
    float scale = ((float_1 - float_0) * ratio + float_0) / 2.f;
    ...
    // Projection Space
    output[0].position = mul(output[0].position, matProjection);
    output[1].position = mul(output[1].position, matProjection);
    output[2].position = mul(output[2].position, matProjection);
    output[3].position = mul(output[3].position, matProjection);
